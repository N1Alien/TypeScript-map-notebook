...
def execute_sql(sql_query):
    # OSTATECZNY ENDPOINT REGIONALNY FRANKFURT (AWS)
    url = "https://neon.tech"
...
class ProductionCloudBackendHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/debug" or self.path == "/debug/":
            ...
            self.end_headers()
            self.wfile.write(json.dumps({"DATABASE_URL_USED": DATABASE_URL[:40] + "...", "TOTAL_LOGS": len(DEBUG_LOGS), "HISTORY": DEBUG_LOGS}, indent=2).encode("utf-8"))
            return
        ...
        if self.path == "/posts" or self.path == "/posts/":
            ...
            try:
                ...
                for r in rows:
                    ...
                    item = {"id": int(p_id), "content": str(p_content), "savedStyle": str(p_style), "coord": {"lat": float(p_lat), "lng": float(p_lng)} if p_lat is not None and p_lng is not None else None, "distance": str(p_dist), "savedIntel": p_intel}
                    output.append(item)
                if not output:
                    self.wfile.write(json.dumps([{"id": 180, "content": "Węzeł Produkcyjny AWS", "savedStyle": "bold", "coord": {"lat": 52.2297, "lng": 21.0122}, "distance": "0", "savedIntel": None}]).encode("utf-8"))
                else:
